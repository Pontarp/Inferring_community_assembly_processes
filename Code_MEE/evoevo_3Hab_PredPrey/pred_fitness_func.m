...
Zmut=Z(end); %Mutant trait

%Set up the death vector for mutant
death_vec=ones(n,1)*d;
...
a=[];
for j=1:m_cons
    a(1,j)=cP*(bmax*exp(-(Zmut-V(j)).^2/2/sigma_b^2)); 
end
...
%Consumer abundance vector
abun_vec_cons = reshape(N,1,m_cons*n)';

%Save the newly formed matrices
parameters.A = A; 
parameters.disp_mat = disp_mat; 
parameters.abun_vec_cons = abun_vec_cons; 
parameters.death_vec = death_vec;
parameters.abun_vec_mut = abun_vec_mut;

%Compute local fitness
tmp = abun_vec_mut.*(death_vec+(A*abun_vec_cons));
mutFit_loc = tmp(mutHab);

%Compute global fitness
tmp = abun_vec_mut.*(death_vec+(A*abun_vec_cons))+(disp_mat*abun_vec_mut);
mutFit_glob = sum(tmp);
...
[t,y] = ode15s( @(t,y) ode_sys_mutfit_predator( t , y , parameters ) , [0,t_end] , y0 , ode_opts ); %Here we use ode15s as it works better than ode45 with the non-negative options
